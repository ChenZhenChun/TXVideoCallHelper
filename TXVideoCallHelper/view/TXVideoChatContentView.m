...
- (IBAction)muteAction:(UIButton *)sender {
    ...
    if (sender.isSelected) {
        //静音
        [sender setImage:[UIImage imageNamed:@"btn_mute_hl"] forState:UIControlStateNormal];
        [[TRTCCloud sharedInstance] muteLocalAudio:YES];
    }else {
        //非静音
        [sender setImage:[UIImage imageNamed:@"btn_mute"] forState:UIControlStateNormal];
...
- (IBAction)audioModeAction:(UIButton *)sender {
    ...
    if (sender.isSelected) {
        //免提（扬声器）
        [sender setImage:[UIImage imageNamed:@"btn_audioMode_hl"] forState:UIControlStateNormal];
        [[TRTCCloud sharedInstance] setAudioRoute:TRTCAudioModeSpeakerphone];
    }else {
        //非免提（听筒）
        [sender setImage:[UIImage imageNamed:@"btn_audioMode"] forState:UIControlStateNormal];
        [[TRTCCloud sharedInstance] setAudioRoute:TRTCAudioModeEarpiece];
    }
}
...
- (void)setPhotoUrl:(NSString *)photoUrl {
    _photoUrl = photoUrl;
    [_photoImgV sd_setImageWithURL:[NSURL URLWithString:photoUrl] placeholderImage:[UIImage imageNamed:@"videoDefaultPhoto"] options:SDWebImageAllowInvalidSSLCertificates completed:^(UIImage * _Nullable image, NSError * _Nullable error, SDImageCacheType cacheType, NSURL * _Nullable imageURL) {
